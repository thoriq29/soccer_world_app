...
import id.thoriq.dev.footballmatch.match.MatchSearchActivity
...
class LeagueActivity : AppCompatActivity() {
    private val leagues = mutableListOf(
        League(
            4328,
        ...
        verticalLayout {
            ...
            recyclerView {
                ...
                adapter = LeagueAdapter(leagues) {
                    startActivity<LeagueDetailActivity>("id" to it.id.toString())
                }
            }
        }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.searchMenuHome -> startActivity<MatchSearchActivity>()
        }
